...
void ICON_Button(const bool selected, const int iconid, const frame_rect_t &ico, const text_info_t (&txt), FSTR_P caption) {
  DWINUI::Draw_Icon(iconid + selected, ico.x, ico.y);
  if (selected) DWINUI::Draw_Box(0, HMI_data.Highlight_Color, ico);
  if (!selected) DWINUI::Draw_Box(0, HMI_data.Background_Color, ico);
  if (HMI_IsChinese()) {
...
void onClickSDItem() {
  ...
  if (hasUpDir && CurrentMenu->selected == 1) return SDCard_Up();
    ...
    const uint16_t filenum = CurrentMenu->selected - 2 - hasUpDir;
  ...
  void FileMenuIdle(bool reset=false) {
    ...
    if (last_itemselected != selected) {
      if (last_itemselected >= 2 + hasUpDir) FileMenu->Items()[last_itemselected]->redraw(true);
      last_itemselected = selected;
      if (selected >= 2 + hasUpDir) {
        const int8_t filenum = selected - 2 - hasUpDir; // Skip "Back" and ".."
        card.selectFileByIndexSorted(filenum);
    ...
    else if ((selected >= 2 + hasUpDir) && (shift_len > MENU_CHAR_LIMIT)) {
...
void onDrawFileName(MenuItemClass* menuitem, int8_t line) {
  ...
  if (is_subdir && menuitem->pos == 2) {
    Draw_Menu_Line(line, ICON_Folder, "..");
  }
  else {
    uint8_t icon;
    card.selectFileByIndexSorted(menuitem->pos - is_subdir - 2);
...
void onDrawHostPrintMenuItem(MenuItemClass* menuitem, int8_t line) {
  Draw_Menu_Line(line, ICON_HotendTemp, "Host Print");
}

void onDrawHostPrintMenuItems(MenuItemClass* menuitem, int8_t line) {
  if (menuitem->pos == 1) {
    Draw_Menu_Line(line, ICON_HotendTemp, "Start Host Print");
  } else if (menuitem->pos == 2) {
    Draw_Menu_Line(line, ICON_Folder, "Pause Host Print");
  } else if (menuitem->pos == 3) {
    Draw_Menu_Line(line, ICON_Folder, "Stop Host Print");
  }
}

void Draw_Host_Print_Menu() {
  checkkey = Menu;
  if (SET_MENU(HostMenu, MSG_HOST_PRINT, 2)) {
    BACK_ITEM(Draw_Print_File_Menu);
    MenuItemAdd(onDrawHostPrintMenuItems, hostui.start);
  }
  UpdateMenu(HostMenu);
  TERN_(DASH_REDRAW, DWIN_RedrawDash());
}
...
void Draw_Print_File_Menu() {
  checkkey = Menu;
  if (card.isMounted()) {
    if (SET_MENU(FileMenu, MSG_MEDIA_MENU, nr_sd_menu_items() + 2)) {
      BACK_ITEM(Goto_Main_Menu);
      MenuItemAdd(onDrawHostPrintMenuItem, Draw_Host_Print_Menu);
    ...
    if (SET_MENU(FileMenu, MSG_MEDIA_MENU, 2)) {
      BACK_ITEM(Goto_Main_Menu);
      MenuItemAdd(onDrawHostPrintMenuItem, Draw_Host_Print_Menu);
    }
    UpdateMenu(FileMenu);
...
void Draw_Info_Menu() {
  ...
  DWINUI::Draw_CenteredString(310, F("Hello Fennex! :)"));
